...
# bind $StartTask to a menu item click handler, button click handler, etc.
$StartTask = {
    # add a script to run in the other thread
    $global:Session = [PowerShell]::Create().AddScript({
        Import-Module (Join-Path $MyPSPath "MultithreadQueue.ps1")
        ...
        for ([int]$n = 1; $n -le 10; $n++) {
            ...
            $SubitemsToProcess = New-Object System.Collections.ArrayList

            <#

            # threaded job processing code here (e.g. build a list of subitems to process)
            # [void]$SubitemsToProcess.Add("something $n")

            #>

            # add the subitems onto the multithreaded queue
            foreach ($subitem in $SubitemsToProcess) {
                ...
                $queue.AddJob({
                    ...
                    try {
                        ...
                        # subitem job processing code here
